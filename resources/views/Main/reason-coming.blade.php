...
<div class="min-h-screen bg-gray-50 flex flex-col justify-center py-4 px-2 sm:px-6 lg:px-8">
    <div class="mx-auto w-full max-w-xs sm:max-w-sm md:max-w-md">
        <h2 class="text-center text-2xl sm:text-3xl font-bold text-gray-800">
            Form Alasan Datang - Non Produktif
        </h2>
    </div>

    <div class="mt-6 mx-auto w-full max-w-xs sm:max-w-sm md:max-w-md">
        <div class="bg-white py-6 px-4 shadow rounded-lg sm:px-6">
            <form class="space-y-4" action="{{ route('reason-coming.store') }}" method="POST">
                @csrf
                
                <input type="hidden" name="token" value="{{ $reasonEntry->token }}" readonly autocomplete="off">

                <div>
                    <label for="current_time" class="block text-xs sm:text-sm font-medium text-gray-700">Jam Datang</label>
                    <div class="mt-1">
                        <input id="current_time" name="current_time" type="text" value="{{ $reasonEntry->time }}" readonly 
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input id="nis" name="nis" type="text" value="{{ $reasonEntry->nis }}" readonly 
                            class="bg-gray-100 w-full px-2 py-1.5 text-xs sm:text-sm border border-gray-300 rounded-md shadow-sm">
                    </div>
                </div>
                ...
                <div>
                    <label for="alasan" class="block text-xs sm:text-sm font-medium text-gray-700">Alasan Datang Hari Ini <span class="font-bold text-red-600">*</span></label>
                    <div class="mt-1">
